	...
	document.addEventListener( 'keypress', function( event ){
		if( event.target.tagName.toLowerCase() !== 'input' &&
			...
			this.keyboardControlsEnabled ){
				...
				var ind = 'XxRrMmLlYyUuEeDdZzFfSsBb'.indexOf( key );
				var Arr = ["X","X'","R","R'","M","M'","L","L'","Y","Y'","U","U'","E'","E","D","D'","Z","Z'","F","F'","S","S'","B","B'"];
				if( ind >= 0 )
				{	
					this.twist( key );
					window.noturn=1;
					window.RUBIK.parse(Arr[ind]);
					window.noturn=0;
				}
		}
	}.bind( this ));

	this.setTwistDuration = (duration = 300) => {
